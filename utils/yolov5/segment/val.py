...
def run(
    ...
    if not training:
        ...
        model.warmup(imgsz=(1 if pt else batch_size, 20, imgsz, imgsz))  # warmup
    ...
    for batch_i, (im, targets, paths, shapes, masks) in enumerate(pbar):
        ...
        for si, (pred, proto) in enumerate(zip(preds, protos)):
            ...
            if paths[si].endswith('.tif') :
                h, w, c = tiff.imread(paths[si]).shape
            elif paths[si].endswith('.jpg') or paths[si].endswith('.jpeg') :
                h, w = cv2.imread(paths[si]).shape
                
            binary_mask = torch.zeros(pred_masks.shape[1:], dtype=torch.float32)
            for mask in pred_masks.cpu() :
            ...
            plt.imsave(f'/datadisk2/c241_ml02/workspace/backend/utils/masks/{file_name}', binary_mask)
